 ...
 */
 ...
 if($_pagi_conteo_alternativo == false){
	...
	$_pagi_result2 = pg_query($conn,$_pagi_sqlConta);
	...
	// Si ocurrió error y mostrar errores está activado
 	if($_pagi_result2 == false && $_pagi_mostrar_errores == true){
		...
		die (" Error en la consulta de conteo de registros: $_pagi_sqlConta. PostgreSQL dijo: <b>".pg_last_error($conn)."</b>");
 	}

 	//$_pagi_totalReg = mysql_result($_pagi_result2,0,0);//total de registros
	list( $_pagi_totalReg ) = pg_fetch_array($_pagi_result2);//total de registros
	
 }else{

	//$_pagi_result3 = mysql_query($_pagi_sql);
	$_pagi_result3 = pg_query($conn,$_pagi_sql);

	// Si ocurrió error y mostrar errores está activado
 	if($_pagi_result3 == false && $_pagi_mostrar_errores == true){
		die (" Error en la consulta de conteo alternativo de registros: $_pagi_sql. Mysql dijo: <b>".pg_last_error($conn)."</b>");
 	}
	//$_pagi_totalReg = mysql_num_rows($_pagi_result3);
	list( $_pagi_totalReg ) = pg_fetch_array($_pagi_result3);//total de registros
 ...
 */
 ...
 //Variable que invente, es opcional, solo la ocupo para una funcion especifica que estoy usando, se podria eliminar, no hay problema
 $_pagi_Lim = " LIMIT $_pagi_cuantos OFFSET $_pagi_inicial";

 //$_pagi_result = mysql_query($_pagi_sqlLim);
 $_pagi_result = pg_query($conn,$_pagi_sqlLim);
 
 // Si ocurrió error y mostrar errores está activado
 if($_pagi_result == false && $_pagi_mostrar_errores == true){
 	die ("Error en la consulta limitada: $_pagi_sqlLim. PostgreSQL dijo: <b>".pg_last_error($conn)."</b>");
 }
